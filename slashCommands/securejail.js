...
const { isJailed, setJailEntry, updateJailEntry } = require("../../utils/jailStorage");
        ...
        try {
            ...
            if (logsChannel) {
                await logsChannel.send({
                    content:
                        "```diff\n" +
                        "+ Membre placé en SecureJail.\n" +
                        `Cible: ${target.user.username} (ID: ${target.id})\n` +
                        `Modérateur: ${interaction.user.username} (ID: ${interaction.user.id})\n` +
                        `Raison: ${reason}\n` +
                        `Salon créé: #${jailChannel.name} (ID: ${jailChannel.id})\n` +
                        `Rôles sauvegardés: ${savedRoles.length}\n` +
                        "```"
                });
            }
        } catch (err) {
            console.error("Erreur logs securejail :", err);
        }

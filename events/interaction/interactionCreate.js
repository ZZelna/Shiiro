...
if (
    interaction.isButton() &&
    interaction.customId.startsWith("survey_vote_")
) {
...
if (alreadyVoted) {
  ...
  // retire l'ancien vote
    ...
    // ajoute le nouveau
    survey.choices[index].votes.push(
        interaction.user.id
    );

    survey.totalVotes = survey.choices.reduce(
...
embed.addFields({
    ...
    embed.addFields({
        name: "👥 Total",
        value: `${survey.totalVotes} vote(s)`
    });

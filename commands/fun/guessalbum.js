...
module.exports = {
    ...
    if (!message.member.roles.cache.has(roleAllowed)) {
        ...
        const question = albums[Math.floor(Math.random() * albums.length)];
        ...
        const embed = new EmbedBuilder()
            .setColor("#5865F2")
            .setTitle("💿 Guess Album")
            .setDescription("Trouve l'album (ou le single) dont est extraite cette musique !\n\n⏱️ Tu as **30 secondes** pour répondre.")
            .setFooter({ text: "Réponds dans le chat" });

        await message.channel.send({ embeds: [embed], files: [audio] });

        // ✅ Liste des réponses valides (nom d'album + alias)
        const validAnswers = [
            question.album.toLowerCase().trim(),

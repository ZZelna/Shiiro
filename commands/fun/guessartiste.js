...
const artistes = require("../../data/artistes.json");
...
module.exports = {
    ...
    run: async (message, args, options = {}) => {
        ...
        const question = artistes[Math.floor(Math.random() * artistes.length)];
        ...
        const embed = new EmbedBuilder()
            .setColor("#5865F2")
            .setTitle("🎤 Guess Artiste")
            .setDescription("Trouve l'artiste qui interprète cette musique !\n\n⏱️ Tu as **30 secondes** pour répondre.")
            .setFooter({ text: "Réponds dans le chat" });

        await message.channel.send({ embeds: [embed], files: [audio] });

        // ✅ Liste des réponses valides (nom d'artiste + alias)
        const validAnswers = [
            question.artist.toLowerCase().trim(),

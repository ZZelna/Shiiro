    ...
    const amount = interaction.options.getInteger("montant");
        ...
        try {
            ...
            return message.reply(
                `❌ Solde insuffisant. Tu as **${(poster?.yens ?? 0).toLocaleString()} ¥**.`
            );
        }

        // Prélèvement des yens
        poster.yens -= amount;
        await poster.save();

        const channel = message.guild.channels.cache.get(BOUNTY_CHANNEL);
        ...
        const bountyMessage = await channel.send({
            embeds: [embed]
        });
              await Bounty.create({
            targetId: target.id,
